...
public:
	...
	void OnFullScreenChanged(bool bFullScreen)
	{
		...
		SetPaneBorder(_imageSplitter, !bFullScreen);
	...
	{
		// 2.0 puts the edge around each complete pane, including the image pane's
		// description panel. 2.2 moved it inward onto each surface.
		...
		_image.Create(_imageSplitter, rcDefault, NULL,
		              WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);
