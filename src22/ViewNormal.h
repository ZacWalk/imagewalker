...
public:
	...
	// The metadata panel is the bottom pane of a splitter inside the image side,
	// so the picture and the panel are sized against each other and not against
	// the items list. 2.3 floats the same information over the picture.
	CSplitter2Window _imageSplitter;
	CDescriptionWindow _description;
	...
	bool InvokeCommand(DWORD id)
	{
		...
		case ID_VIEW_FILMSTRIP:
			SetSinglePaneMode(GetSinglePaneMode() == SPLIT_PANE_NONE ? SPLIT_PANE_TOP : SPLIT_PANE_NONE);
			return true;
	...
	bool GetCommandState(DWORD id, bool &bEnabled, bool &bChecked)
	{
		...
		// Only full screen has a film strip to hide; the windowed splitter is
		// dragged instead, and hiding a pane there would strand the items view.
		case ID_VIEW_FILMSTRIP:
			bEnabled = _pCoupling->IsFullScreen();
			bChecked = bEnabled && GetSinglePaneMode() == SPLIT_PANE_NONE;
			return true;
	...
	// The gutter strip is not a toolbar the frame owns, so it is told what the
	// idle pass decided rather than working the answer out a second time.
	void OnCommandStateKnown(WORD id, bool bEnabled, bool bChecked) override
	{
		_image.SetGutterState(id, bEnabled, bChecked);
	}
	...
	// The panel is up whenever the user asked for it, and always in metadata
	// mode. One call decides it, so the two cannot disagree.
	void ShowDescriptionPanel(bool bShow)
	{
		if (_imageSplitter.m_hWnd == nullptr)
			return;

		const bool bShown = _imageSplitter.GetSinglePaneMode() == SPLIT_PANE_NONE;

		if (bShown == bShow)
			return;

		if (!bShow)
			App.Settings.DescriptionSplitterPos = _imageSplitter.GetProportionalPos();

		_imageSplitter.SetSinglePaneMode(bShow ? SPLIT_PANE_NONE : SPLIT_PANE_TOP);

		if (bShow)
		{
			_imageSplitter.SetProportionalPos(App.Settings.DescriptionSplitterPos);
			_description.Refresh();
		}
	}
	...
	void OnFullScreenChanged(bool bFullScreen)
	{
		...
		// The sunken border is what separates two panes on a lit frame. Full
		// screen is one black surface, and an edge drawn across it reads as a
		// window that failed to go full screen.
		SetPaneBorder(_image, !bFullScreen);
		SetPaneBorder(_folder, !bFullScreen);

		_folder.SizeClients();
		SetSplitterRect();
	}

	static void SetPaneBorder(CWindow wnd, bool bBorder)
	{
		if (wnd.m_hWnd == nullptr)
			return;

		wnd.ModifyStyleEx(bBorder ? 0 : WS_EX_CLIENTEDGE,
	...
	{
		// The sunken border round each pane is 2.2's, not 2.3's, which is
		// borderless either side of a drawn splitter. See docs/architecture-2x.md.
		_folder.Create(m_hWnd, rcDefault, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
		               WS_EX_CLIENTEDGE);

		_imageSplitter.Create(m_hWnd, rcDefault, NULL,
		                      WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);

		_image.Create(_imageSplitter, rcDefault, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
		              WS_EX_CLIENTEDGE);
		_description.Create(_imageSplitter, rcDefault, NULL,
		                    WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
		                    WS_EX_CLIENTEDGE);

		_imageSplitter.SetOrientation(false, false);
		_imageSplitter.SetSplitterPanes(_image, _description);
		_imageSplitter.SetProportionalPos(App.Settings.DescriptionSplitterPos);
		_imageSplitter.SetSinglePaneMode(SPLIT_PANE_TOP);
		...
		SetSplitterPanes(_imageSplitter, _folder);
	...
	{	
		...
		if (_imageSplitter.GetSinglePaneMode() == SPLIT_PANE_NONE)
			_description.Refresh();
		UpdateStatusText();
	}	
