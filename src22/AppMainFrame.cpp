...
bool CMainFrame::CreateToolBars()
{
	...
	_modeBar.Create(m_hWnd, rcDefault, nullptr, WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS);
...
void CMainFrame::UpdateLayout(BOOL bResizeBars)
{
	...
	{
		// The mode bar takes the left edge before the client gets what is left.
		if (_modeBar.m_hWnd != nullptr && IW::HasVisibleStyle(_modeBar))
		{
			const int cxBar = IW::Min(static_cast<int>(CModeBar::kWidth), rectClient.Width());

			_modeBar.SetWindowPos(nullptr, rectClient.left, rectClient.top, cxBar, rectClient.Height(),
			                      SWP_NOZORDER | SWP_NOACTIVATE);
			rectClient.left += cxBar;
		}
...
void CMainFrame::SetMode(Mode mode)
{
	...
	switch (mode)
	{
	case Mode::Folders: _modeBar.SetMode(ID_VIEW_FOLDERS); break;
	case Mode::Search: _modeBar.SetMode(ID_VIEW_SEARCHADVANCED); break;
	case Mode::Edit: _modeBar.SetMode(ID_VIEW_EDIT); break;
	case Mode::Print: _modeBar.SetMode(ID_VIEW_PRINT); break;
	case Mode::FullScreen: _modeBar.SetMode(ID_VIEW_IMAGEFULLSCREEN); break;
	case Mode::Metadata: _modeBar.SetMode(ID_VIEW_DESCRIPTION); break;
	default: _modeBar.SetMode(ID_VIEW_NORMAL); break;
	}
...
void CMainFrame::UpdateToolbarsShown()
{
	...
	// The mode bar is the left edge of the chrome, so it goes with the rest of
	// it. Escape, F11 and the gutter's own button are the way back.
	_modeBar.ShowWindow(bShowStatus ? SW_SHOW : SW_HIDE);
