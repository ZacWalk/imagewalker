...
//
// Purpose: CRender implementation and the palette IW::Style::SetPalette
//          seeds.
//
...
// 2.2 wears the system's colours, so the selection is whatever the user's theme
// says -- blue on a stock Windows. The mood palette and the black background
// that make 2.3 dark are 2.3's alone; see docs/architecture-2x.md.
void IW::Style::SetPalette()
{
	Color::Window = GetSysColor(COLOR_WINDOW);
	...
	Color::TaskText = GetSysColor(COLOR_WINDOWTEXT);
	Color::TaskTextBold = GetSysColor(COLOR_WINDOWTEXT);
	Color::TaskBackground = GetSysColor(COLOR_3DFACE);
	Color::TaskFrame = GetSysColor(COLOR_3DSHADOW);

	Color::MenuBackground = GetSysColor(COLOR_MENU);
