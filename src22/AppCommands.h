...
public:
	...
	// The answer the idle pass settled on, handed back so a control outside the
	// menus and toolbars can reflect it rather than keep its own copy.
	virtual void OnCommandStateKnown(WORD id, bool bEnabled, bool bChecked) {};
};
...
public:	
	...
	HWND CreateToolbar(HWND hwndParent, int nId, LPTBBUTTON lpButtons, int nCount, CString strings = CString())
	{
		DWORD dwStyle = WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | 
			...
			TBSTYLE_TOOLTIPS | TBSTYLE_LIST | TBSTYLE_FLAT;
	...
	{
		T *pT = static_cast<T*>(this);		const _AtlUpdateUIMap* pMap = m_pUIMap;
		_AtlUpdateUIData* pUIData = m_pUIData;
		if(pUIData != NULL)
		{
			for( ; pMap->m_nID != (WORD)-1; pMap++, pUIData++)
			{
				...
				pT->OnCommandStateKnown(pMap->m_nID, bEnable, bChecked);
