...
public:
	...
	CImageWindowImpl(Coupling *pCoupling, State &state) : 
	  ...
	  // The four-square pinwheel in the middle of an empty pane: green at the
	  // top, yellow left, cyan right, red at the bottom, which is the layout of
	  // the 1998 app icon. 2.3 commented this out and shows nothing.
	  void DrawLogo(IW::CRender &render)
	  {
		  CRect rectClient;
		  ::GetClientRect(static_cast<T*>(this)->m_hWnd, rectClient);

		  if (rectClient.IsRectEmpty())
			  return;

		  const int nLogoSize = IW::Min(60, IW::Min(rectClient.Width(), rectClient.Height()) / 4);

		  if (nLogoSize < 8)
			  return;

		  const CPoint ptCentre = rectClient.CenterPoint();
		  const int cy = MulDiv(nLogoSize * 2, 10, 45);
		  const COLORREF clrBorder = RGB(0x40, 0x40, 0x40);

		  const CRect rectGreen(ptCentre.x - cy, ptCentre.y - nLogoSize,
		                        ptCentre.x + cy, ptCentre.y - nLogoSize + (2 * cy));
		  const CRect rectCyan(ptCentre.x + nLogoSize - (2 * cy), ptCentre.y - cy,
		                       ptCentre.x + nLogoSize, ptCentre.y + cy);
		  const CRect rectRed(ptCentre.x - cy, ptCentre.y + nLogoSize - (2 * cy),
		                      ptCentre.x + cy, ptCentre.y + nLogoSize);
		  const CRect rectYellow(ptCentre.x - nLogoSize, ptCentre.y - cy,
		                         ptCentre.x - nLogoSize + (2 * cy), ptCentre.y + cy);

		  render.DrawRect(rectGreen, clrBorder, RGB(0x08, 0xd6, 0x08), 2);
		  render.DrawRect(rectCyan, clrBorder, RGB(0x18, 0xd6, 0xd6), 2);
		  render.DrawRect(rectRed, clrBorder, RGB(0xd6, 0x18, 0x18), 2);
		  render.DrawRect(rectYellow, clrBorder, RGB(0xde, 0xde, 0x08), 2);

		  CRect rectText(rectClient.left, ptCentre.y + nLogoSize + 8,
		                 rectClient.right, ptCentre.y + nLogoSize + 32);

		  // IDR_MAINFRAME's string is the app name and version.
		  render.DrawString(App.LoadString(IDR_MAINFRAME), rectText, IW::Style::Font::Heading,
	  ...
	  int GetScale() const
	  {
		  ...
		  CRect rectWindow;
		  pT->GetClientRect(rectWindow);
		  return _scale.CalcScalePercent(sizeImage, pT->GetImageClientRect().Size(), rectWindow.Size());
	  }

	  CSize CalcImageSize(const CSize &sizeClient) const
	  {
		  const T *pT = static_cast<const T*>(this);
		  CRect rectWindow;
		  pT->GetClientRect(rectWindow);
		  return _scale.CalcSize(_state.Image.GetDisplaySize(), sizeClient, rectWindow.Size());
	  }
	  ...
	  {	
		  ...
		  _sizeClient = rectImageDisplay.Size();

		  CSize sizeDrawnImage = CalcImageSize(pT->GetImageClientRect().Size());
	  ...
	  void ScrollTo(const CPoint &point)
	  {
		  ...
		  pT->OnScrollRangeChanged();
		  pT->Invalidate();
	  }

	  // Only the browse pane has real scroll bars to keep in step; the edit
	  // and print panes scroll the same model with nothing showing it.
	  ...
	  // Only the browse pane raises these; the edit and print panes have no gutter.
	  void TrackGutterMenu(CMenuHandle, const CRect &)
	  {
	  }
	  ...
	  void Refresh(bool bScrollToCenter)
	  {
		  ...
		  // A new picture at the same scale mode is a new percentage, and the
		  // readout is the only thing that says what it is.
		  pT->OnScaleChange();
	  }
	  ...
	  {
		  // Classic profiles use the toolbar popup, never an image overlay.
		  return false;
	  }
};
