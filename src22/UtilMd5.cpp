...
//
// Based on the C++ MD5 translation by Mordechai T. Abzug, of the RSA Data
//          Security, Inc. MD5 Message-Digest Algorithm. The original notice
//          below applies.
//
...
MD5::MD5(const char* string)
{
	init(); // must be called be all constructors
	update((uint1*)string, static_cast<unsigned int>(strlen(string)));
	finalize();
}
...
{
	stream << CStringA(context.hex_digest()).GetString();
	return stream;
}
